...
export default function Integrations() {
  ...
  const serviceCards = [
    { name: 'Retell AI', key: 'retell', icon: Zap, color: '#3B82F6', desc: 'Voice AI agent' },
    { name: 'Twilio', key: 'twilio', icon: Phone, color: '#DC2626', desc: 'SMS & calls' },
    { name: 'Cal.com', key: 'calcom', icon: Calendar, color: '#16A34A', desc: 'Appointment booking' },
    { name: 'Telegram', key: 'telegram', icon: MessageSquare, color: '#0088CC', desc: 'Owner notifications' },
    { name: 'SMTP', key: 'smtp', icon: Mail, color: '#EAB308', desc: 'Email outreach' },
  ...
  return (
    <div className="fade-in">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 600 }}>Integrations</h1>
              ...
              <div key={svc.key} className="card" style={{ padding: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#555' }}>{svc.desc}</div>
                  </div>
                </div>
                <StatusBadge
      ...
      <section style={{ marginBottom: 40 }}>
        ...
        <div className="card" style={{ padding: 20 }}>
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#888', marginBottom: 8 }}>Inbound (set these in external dashboards)</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                ...
                <div key={wh.label} style={{
                  ...
                  padding: '8px 12px', background: '#0d0d0d', borderRadius: 6,
                }}>
                  <div>
                    <span style={{ fontSize: 11, color: '#555', marginRight: 8 }}>{wh.label}</span>
                    <code style={{ fontSize: 12, color: '#e0d8c8', fontFamily: 'monospace' }}>{wh.url}</code>
                  </div>
                  <button
                    className="btn-ghost"
                  ...
                  >
                    {copied === wh.label ? <><Check size={11} color="#16A34A" /> Copied</> : <><Copy size={11} /> Copy</>}
                  </button>
                </div>
              ))}
            </div>
          </div>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#888', marginBottom: 4 }}>Outbound (configure per client in Settings)</div>
          <div style={{ fontSize: 12, color: '#555' }}>
            {integrations?.webhooks ? (
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 4 }}>
                {Object.entries(integrations.webhooks).map(([key, configured]) => (
                  <span key={key} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <div style={{ width: 6, height: 6, borderRadius: '50%', background: configured ? '#16A34A' : '#DC2626' }} />
                    {key.replace(/_/g, ' ')}
                  </span>
                ))}
              </div>
      ...
      <section style={{ marginBottom: 40 }}>
        ...
        <div className="card" style={{ padding: 20 }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
            <div>
              <label style={{ fontSize: 11, color: '#555', display: 'block', marginBottom: 4 }}>Event Type</label>
              <select
                value={testEvent}
                onChange={e => setTestEvent(e.target.value)}
                style={{
                  padding: '8px 12px', background: '#141414', border: '1px solid #222',
                  borderRadius: 6, color: '#e0d8c8', fontSize: 13, minWidth: 200,
                }}
              >
                {EVENT_TYPES.map(et => (
          ...
          <div style={{ marginTop: 8, fontSize: 11, color: '#555' }}>
            Sends a test event with <code style={{ color: '#888' }}>_test: true</code> to your configured webhook URL. Check the delivery log below for results.
          </div>
        </div>
      </section>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid #141414' }}>
                  {['Event', 'URL', 'Attempts', 'Status', 'Time'].map(col => (
                    <th key={col} style={{
                      textAlign: 'left', fontWeight: 600, fontSize: 12, color: '#888',
              ...
              <tbody>
                {webhookLog.map(entry => (
                  <tr key={entry.id} style={{ borderBottom: '1px solid #0d0d0d' }}>
                    <td style={{ padding: '12px 16px', fontSize: 12, color: '#C9A84C', fontWeight: 600 }}>
                      {entry.event}
                    </td>
                    <td style={{ padding: '12px 16px', fontSize: 12, color: '#888', fontFamily: 'monospace', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {entry.url}
                    </td>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      {entry.status === 'failed' ? (
                        ...
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: '#EAB308', fontSize: 12 }}>
      ...
      <section>
        <h2 style={{ fontSize: 16, fontWeight: 600, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
          <FileSpreadsheet size={18} color="#16A34A" /> Google Sheets Integration
        </h2>
        <div className="card" style={{ padding: 20 }}>
          <div style={{ fontSize: 13, color: '#e0d8c8', marginBottom: 16 }}>
            Sync leads, calls, and messages to Google Sheets in real-time using Zapier or Make.
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#C9A84C', marginBottom: 6 }}>Option 1: Zapier (recommended)</div>
              <ol style={{ fontSize: 12, color: '#888', paddingLeft: 20, lineHeight: 1.8 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#C9A84C', marginBottom: 6 }}>Option 2: Make (Integromat)</div>
              <ol style={{ fontSize: 12, color: '#888', paddingLeft: 20, lineHeight: 1.8 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#C9A84C', marginBottom: 6 }}>Option 3: CSV Export (manual)</div>
          ...
          <div style={{
            marginTop: 16, padding: '12px 16px', background: '#141414', borderRadius: 6,
            fontSize: 12, color: '#555',
          }}>
            <strong style={{ color: '#888' }}>Webhook payload format:</strong>
            <pre style={{ marginTop: 6, color: '#888', fontFamily: 'monospace', fontSize: 11, overflow: 'auto' }}>
{`{
  "event": "call_ended",
  "clientId": "uuid",
