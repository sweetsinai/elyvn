...
describe('createJobHandlers', () => {
  it('returns an object with all expected job type keys', () => {
    ...
    expect(handlers).toHaveProperty('interested_followup_email');
    expect(handlers).toHaveProperty('noreply_followup');
...
describe('interestedFollowupEmail', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    captureException = jest.fn();
    mockTransport = { sendMail: jest.fn().mockResolvedValue({ messageId: 'ok' }) };
    getTransporter.mockReturnValue(mockTransport);
  });

  it('sends follow-up email to interested prospect', async () => {
    db = makeDb({
      'SELECT * FROM prospects': { id: 'p1', status: 'interested', phone: '+1', business_name: 'Biz Co' },
      'SELECT 1 FROM appointments': undefined,
      'SELECT id FROM emails_sent': undefined,
    });

    await interestedFollowupEmail(db, captureException, {
      prospect_id: 'p1',
    ...
    expect(mockTransport.sendMail).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        from: 'sender@elyvn.com',
      })
    );
  });

  it('skips when prospect already booked', async () => {
    db = makeDb({
      'SELECT * FROM prospects': { id: 'p1', status: 'booked' },
    });

    await interestedFollowupEmail(db, captureException, { prospect_id: 'p1', to_email: 'x@x.com' });

    expect(mockTransport.sendMail).not.toHaveBeenCalled();
    expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('already booked'));
  });

  it('skips when prospect not found', async () => {
    db = makeDb({ 'SELECT * FROM prospects': undefined });
  ...
  it('skips when prospect has a booking', async () => {
    db = makeDb({
      'SELECT * FROM prospects': { id: 'p1', status: 'interested', phone: '+1' },
      'SELECT 1 FROM appointments': { 1: 1 },
    });

    await interestedFollowupEmail(db, captureException, { prospect_id: 'p1', to_email: 'x@x.com' });

    expect(mockTransport.sendMail).not.toHaveBeenCalled();
    expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('has a booking'));
  });

  it('skips when there is a recent duplicate email', async () => {
    db = makeDb({
      'SELECT * FROM prospects': { id: 'p1', status: 'interested', phone: '+1' },
      'SELECT 1 FROM appointments': undefined,
      'SELECT id FROM emails_sent': { id: 'e1' },
    });
  ...
  it('skips and logs when SMTP transporter not configured', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('SMTP not configured'));
  });

  it('calls captureException on sendMail error', async () => {
...
describe('noreplyFollowup', () => {
  ...
  const basePayload = {
    ...
    from_email: 'f@elyvn.com',
    original_subject: 'Are you catching every call?',
    day: 3,
    campaign_id: 'camp1',
  ...
  it('sends Day 3 follow-up and enqueues Day 7', async () => {
    db = makeDb({
      'SELECT * FROM prospects': baseProspect,
      'SELECT 1 FROM emails_sent WHERE prospect_id': undefined,
      'SELECT id FROM emails_sent WHERE to_email': undefined,
    });

    await noreplyFollowup(db, captureException, basePayload);
  ...
  it('sends Day 7 follow-up without enqueuing further', async () => {
    db = makeDb({
      'SELECT * FROM prospects': baseProspect,
      'SELECT 1 FROM emails_sent WHERE prospect_id': undefined,
      'SELECT id FROM emails_sent WHERE to_email': undefined,
    ...
    await noreplyFollowup(db, captureException, { ...basePayload, day: 7 });

    expect(mockTransport.sendMail).toHaveBeenCalled();
    expect(enqueueJob).not.toHaveBeenCalled();
  });

  it('skips when prospect already replied', async () => {
    db = makeDb({
      'SELECT * FROM prospects': baseProspect,
      'SELECT 1 FROM emails_sent WHERE prospect_id': { 1: 1 },
    });

    await noreplyFollowup(db, captureException, basePayload);

    expect(mockTransport.sendMail).not.toHaveBeenCalled();
    expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('prospect replied'));
  });

  it('skips when prospect status is bounced', async () => {
    db = makeDb({ 'SELECT * FROM prospects': { ...baseProspect, status: 'bounced' } });
  ...
  it('skips when prospect status is unsubscribed', async () => {
    db = makeDb({ 'SELECT * FROM prospects': { ...baseProspect, status: 'unsubscribed' } });
  ...
  it('skips for interested prospect status', async () => {
    db = makeDb({ 'SELECT * FROM prospects': { ...baseProspect, status: 'interested' } });
  ...
  it('skips when recent duplicate email exists', async () => {
    db = makeDb({
      'SELECT * FROM prospects': baseProspect,
      'SELECT 1 FROM emails_sent WHERE prospect_id': undefined,
      'SELECT id FROM emails_sent WHERE to_email': { id: 'e1' },
    });
  ...
  it('skips and logs when SMTP not configured', async () => {
    getTransporter.mockReturnValue(null);
    db = makeDb({
      'SELECT * FROM prospects': baseProspect,
      'SELECT 1 FROM emails_sent WHERE prospect_id': undefined,
  ...
  it('calls captureException on error', async () => {
    mockTransport.sendMail.mockRejectedValue(new Error('Fail'));
    db = makeDb({
      'SELECT * FROM prospects': baseProspect,
      'SELECT 1 FROM emails_sent WHERE prospect_id': undefined,
      'SELECT id FROM emails_sent WHERE to_email': undefined,
