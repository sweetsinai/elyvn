...
router.get('/referral/:clientId', async (req, res, next) => {
  try {
...
router.post('/referral/apply', async (req, res, next) => {
  try {
    ...
    if (!referrer) {
      return res.json({ applied: false, reason: 'Invalid referral code' });
    }

    // Prevent self-referral
    if (referrer.id === new_client_id) {
      return res.json({ applied: false, reason: 'Cannot refer yourself' });
    }
    ...
    if (existing) {
      return res.json({ applied: false, reason: 'Referral already recorded' });
    }
...
router.post('/referral/:clientId/activate', async (req, res, next) => {
  try {
    const db = req.app.locals.db;
    const { clientId } = req.params;
    if (!isValidUUID(clientId)) return next(new AppError('INVALID_INPUT', 'Invalid client ID', 400));
    ...
    if (!claimed || claimed.changes === 0) return res.json({ activated: false, reason: 'No pending referral' });
    ...
    if (!referral) return res.json({ activated: false, reason: 'Referral not found' });
    await db.query(
