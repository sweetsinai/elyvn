...
// Circuit breakers for external APIs (with fallback responses when circuit is open)
...
router.post('/', async (req, res) => {
  ...
  // Nonce / replay prevention — deduplicate within the webhook retry window
  ...
  if (webhookNonce && webhookNonce !== '-') {
    ...
    await addNonce(webhookNonce, 3600); // 1 hour TTL
  }
  ...
  setImmediate(async () => {
    try {
      switch (event) {
        case 'call_started':
          await handleCallStarted(db, call);
          break;
        case 'call_ended':
          await handleCallEnded(db, call);
          break;
        case 'call_analyzed':
          await handleCallAnalyzed(db, call);
          break;
        case 'agent_transfer':
        case 'transfer_requested':
          await handleTransfer(db, call);
          break;
        case 'dtmf':
          if (call && call.digit === '*') await handleTransfer(db, call);
          break;
        default:
          logger.info(`[retell] Unhandled event: ${event}`);
      }
      try {
        const { webhookProcessed, callsTotal } = require('../utils/prom-metrics');
        webhookProcessed.labels('retell', 'success').inc();
        if (event === 'call_started' || event === 'call_ended' || event === 'call_analyzed') {
          callsTotal.labels(event, call?.direction || 'inbound').inc();
        }
      } catch (_) {}
    } catch (err) {
      logger.error('[retell] Processing error', { code: 'PROCESSING_ERROR', event, callId: call?.call_id, error: err.message });
      try {
        const { webhookProcessed } = require('../utils/prom-metrics');
        webhookProcessed.labels('retell', 'error').inc();
      } catch (_) {}
    }
  });
});

async function handleCallStarted(db, call) {
  try {
    ...
    if (!client && call.agent_id) {
      client = await db.query('SELECT id FROM clients WHERE retell_agent_id = ?', [call.agent_id], 'get');
    }
...
async function handleCallEnded(db, call) {
  try {
    ...
    if (!callRecord) {
      ...
      if (!client && agentId) {
        client = await db.query('SELECT id FROM clients WHERE retell_agent_id = ?', [agentId], 'get');
      }
    ...
    if (callerPhone && clientId) {
      ...
      `, [randomUUID(), clientId, callerPhone, score], 'run');

      // 6a. Fire-and-forget event emission for lead lifecycle
      try {
      ...
      if (outcome === 'voicemail') {
        try {
          const voicemailLead = await db.query('SELECT id FROM leads WHERE phone = ? AND client_id = ?', [callerPhone, clientId], 'get');
...
async function handleTransfer(db, call) {
  try {
    ...
    // Fetch transcript (with timeout, via circuit breaker)
    let retellResp = null;
    try {
      ...
      logger.warn(`[retell] handleTransfer: Retell fetch failed for ${callId}:`, fetchErr.message);
    }
