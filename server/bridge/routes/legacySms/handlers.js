...
async function handleInboundSMS(db, { from, to, body, messageId }) {
  try {
    logger.info(`[legacySms] SMS from ${from ? from.replace(/\d(?=\d{4})/g, '*') : '?'} to ${to} (${(body || '').length} chars)`);

    // Fast nonce check (Redis or in-memory) before hitting the DB
    if (messageId && await hasNonce(messageId)) {
      logger.warn(`[legacySms] Duplicate webhook rejected: ${messageId}`);
      return;
    }
    if (messageId) {
    ...
    if (messageId) {
      ...
      if (dup) {
        logger.info(`[legacySms] Duplicate messageId ${messageId}, skipping`);
        return;
      }
    }
    ...
    if (!client) {
      logger.error(`[legacySms] No client found for number ${to}`);
      return;
    }
    ...
    logger.error('[legacySms] handleInboundSMS error:', err);
  }
}
...
async function handleOptOut(db, client, from, to, keyword) {
  try {
    ...
    logger.info(`[legacySms] Recorded opt-out for ${from} (${keyword})`);
  } catch (err) {
    logger.error('[legacySms] handleOptOut error:', err);
  }
}
...
async function handleOptIn(db, client, from, to) {
  try {
    ...
    logger.info(`[legacySms] Recorded opt-in for ${from}`);
  } catch (err) {
    logger.error('[legacySms] handleOptIn error:', err);
  }
}
...
async function handleCancel(db, client, from, replyFrom) {
  try {
    ...
    if (result.success) {
      ...
      logger.info(`[legacySms] Booking ${lead.calcom_booking_id} cancelled for ${from}`);
    } else {
      await sendSMS(from, 'Sorry, we couldn\'t cancel your appointment right now. Please call us directly.', replyFrom, db, client.id);
    }
  } catch (err) {
    logger.error('[legacySms] handleCancel error:', err);
    await sendSMS(from, 'Sorry, something went wrong. Please call us directly.', replyFrom, db, client.id).catch(e => logger.warn('[legacySms] Error SMS send failed', e.message));
  }
}
...
async function handleYes(db, client, from, replyFrom) {
  try {
    ...
    logger.error('[legacySms] handleYes error:', err);
  }
}
