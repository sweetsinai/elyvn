...
router.get('/me', async (req, res, next) => {
  ...
  try {
    const client = await db.query(
      'SELECT id, name, owner_name, owner_email, owner_phone, plan, subscription_status, stripe_customer_id, industry, email_verified, created_at FROM clients WHERE id = ?',
      [payload.clientId],
      'get'
    );
