...
router.post('/', validateBody(SignupSchema), async (req, res, next) => {
  ...
  try {
    ...
    await db.query(`
      INSERT INTO clients (id, name, owner_name, owner_email, owner_phone, password_hash, is_active, plan, email_verified, verification_token, verification_expires, onboarding_step, onboarding_completed, referral_code, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, 1, 'trial', 0, ?, ?, 0, 0, ?, datetime('now'), datetime('now'))
