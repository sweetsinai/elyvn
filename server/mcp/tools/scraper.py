...
async def scrape_businesses(
    ...
    try:
            ...
            try:
                ...
                if website:
                    try:
                        ...
                        logger.debug("Failed to scrape email from %s: %s", website, e)
                ...
                try:
                    ...
                    logger.error("Failed to insert prospect %s: %s", name, e)
                    errors += 1
        ...
        logger.error("scrape_businesses failed: %s", e)
        return {"error": f"Failed to scrape businesses: {str(e)}"}
...
async def _scrape_email_from_website(url: str) -> Optional[str]:
    ...
    try:
        try:
            ...
            logger.debug("HTTP error scraping email from %s: %s", url, e)
        except Exception as e:
            logger.debug("Failed to scrape email from %s: %s", url, e)

        return None
    except Exception as e:
        logger.debug("Unexpected error in email scraping from %s: %s", url, e)
        return None
